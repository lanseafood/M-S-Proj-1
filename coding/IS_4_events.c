...
static int IS_4_red_right_turn( Direction D );
...
static int IS_4_red_right_turn( Direction D ) {
    int redRightTurn = 0;
    int rightTurnLane = get_numLanes( IS_4 )[D];
    if( get_list_counter( get_lane_queue( IS_4, D, rightTurnLane ) ) == 0 ) return 0;
    if( get_lane_flag( IS_4, D, rightTurnLane ) == 1 ) return 0;
    
    Event E = peek_from_list( get_lane_queue( IS_4, D, rightTurnLane ) );
    ...
    switch( D ) {
        ...
        case SOUTH:
        {
            if(   get_lane_counter( IS_4, NORTH, 1 ) == 0
               ) redRightTurn = 1;
            break;
        }
        default:
            fprintf(stderr,"Error from IS_4_red_right_turn(): invalid direction\n"); exit(1);
    }
    return redRightTurn;
}
...
static int IS_4_left_turn( Direction D ) {
    ...
    if( get_protected( IS_4 ) == 1 ) return 1;
    ...
    switch( D ) {
        case NORTH:
        {
            if(   get_lane_counter( IS_4, SOUTH, 1 ) == 0
               && get_lane_counter( IS_4, SOUTH, 2 ) == 0
               && get_lane_counter( IS_4, SOUTH, 3 ) == 0
               && get_lane_counter( IS_4, EAST, 1 ) == 0
               ) leftTurn = 1;
            break;
        }
        case  EAST:
...
static void IS_4_arrival( void* P ) {
       ...
       && get_lane_flag( IS_4, dir, newLane ) == 0 )
    {
        ...
        if( ! ( get_route( V ) == LEFT && IS_4_left_turn( dir ) == 0 ) ) { //fix the L/R turns, b/c no traffic lights for un/protected turns.
            schedule_event( E );
...
static void IS_4_crossing( void* P ) {
    ...
    if( get_list_counter( get_lane_queue( IS_4, dir, laneID ) ) > 0 ) {
        ...
        if( laneID == get_numLanes( IS_4 )[dir] && IS_4_red_right_turn( dir ) ) {
            set_timestamp( nextEvent, get_sim_time() );
            schedule_event( nextEvent );
        } else ;
    }
...
static void IS_4_departure( void* P ) {
    ...
    if( get_route( V ) != LEFT ) {
        ...
        if( IS_4_left_turn( opposite ) == 1 ) {
            Event entering = peek_from_list( get_lane_queue( IS_4, opposite, 1 ) );
    ...
    if( IS_4_red_right_turn( redTurn ) ) {
        Event rightTurn = peek_from_list( get_lane_queue( IS_4, redTurn, get_numLanes( IS_4 )[redTurn] ) );
