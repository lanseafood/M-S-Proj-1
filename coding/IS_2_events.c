...
static int IS_2_choose_route_and_lane( Direction dir, int dest, Vehicle V );
static int IS_2_red_right_turn( Direction D );
static int IS_2_left_turn( Direction D );
...
static int IS_2_choose_route_and_lane( Direction dir, int dest, Vehicle V ) {
	...
	switch ( dir ) {
		case NORTH:
		{
			if( dest == 222 ) { set_route( V, RIGHT ); set_departDir( V, WEST ); laneID = 3; }
			else if( dest == 203 ) { set_route( V, LEFT ); set_departDir( V, EAST ); laneID = 1; }
			else { set_route( V, STRAIGHT ); set_departDir( V, SOUTH ); laneID =
				( get_list_counter( get_lane_queue( IS_2, NORTH, 3 ) ) <
				 get_list_counter( get_lane_queue( IS_2, NORTH, 2 ) ) ) ? 3 : 2; }
			break;
		}
		case EAST:
		{
		...
		case WEST:
		{
			if( dest == 203 ) { set_route( V, STRAIGHT ); set_departDir( V, EAST ); laneID = 1; }
			else if( dest <= 202 || dest == 223 ) { set_route( V, RIGHT ); set_departDir( V, SOUTH ); laneID = 2; }
			else { set_route( V, LEFT ); set_departDir( V, NORTH ); laneID = 1; }
			break;
		}
		default:
			fprintf(stderr,"Error from IS_2_choose_route_and_lane(): unexpected vehicle direction\n"); exit(1);
	}
	return laneID;
}

//////////////////////////////////////////////////////////////////////////////////////////////
// Check if red right turn for direction D is possible based on IS 2 traffic
static int IS_2_red_right_turn( Direction D ) {
	int redRightTurn = 0;
	int rightTurnLane = get_numLanes( IS_2 )[D];
	if( get_list_counter( get_lane_queue( IS_2, D, rightTurnLane ) ) == 0 ) return 0;
	...
	switch( D ) {
		case NORTH:
		{
			if( get_lane_counter( IS_2,  EAST, 1 ) == 0 ) redRightTurn = 1;
			break;
		}
		case  EAST:
		{
			if(   get_lane_counter( IS_2,  WEST, 1 ) == 0
			   && get_lane_counter( IS_2, SOUTH, 1 ) == 0
			   && get_lane_counter( IS_2, SOUTH, 2 ) == 0
			   ) redRightTurn = 1;
			break;
		}
		case SOUTH:
		{
			if(   get_lane_counter( IS_2, NORTH, 1 ) == 0
			   && get_lane_counter( IS_2,  WEST, 1 ) == 0
			   ) redRightTurn = 1;
			break;
		}
		case  WEST:
		{
			if(   get_lane_counter( IS_2,  EAST, 1 ) == 0
			   && get_lane_counter( IS_2, NORTH, 2 ) == 0
			   && get_lane_counter( IS_2, NORTH, 3 ) == 0
			   ) redRightTurn = 1;
			break;
		}
		default:
			fprintf(stderr,"Error from IS_2_red_right_turn(): invalid direction\n"); exit(1);
	}
	return redRightTurn;
}

//////////////////////////////////////////////////////////////////////////////////////////////
// Check if left turn for direction D is possible based on IS 2 traffic
static int IS_2_left_turn( Direction D ) {
	int leftTurn = 0;
	...
	if( get_light( IS_2, D, 1 ) == RED ) return 0;
	if( get_lane_flag( IS_2, D, 1 ) == 1 ) return 0;
	if( get_protected( IS_2 ) == 1 ) return 1;
	
	switch( D ) {
		case NORTH:
		{
			if(   get_lane_counter( IS_2, SOUTH, 1 ) == 0
			   && get_lane_counter( IS_2, SOUTH, 2 ) == 0
			   ) leftTurn = 1;
			break;
		}
		...
		case  WEST:
		{
			if( get_lane_counter( IS_2, EAST, 1 ) == 0 ) leftTurn = 1;
			break;
		}
		default:
			fprintf(stderr,"Error from IS_2_left_turn(): invalid direction\n"); exit(1);
	}
	return leftTurn;
}
...
static void IS_2_arrival( void* P ) {
	...
	set_event_type( E, IS_2_ENTERING );
	set_callback( E, IS_2_entering );
	
	// Queue up or enter intersection depending on vehicle destination
	int newLane = IS_2_choose_route_and_lane( dir, dest, V );
	
	set_laneID( V, newLane );
	add_to_list( get_lane_queue( IS_2, dir, newLane ), E );
	// Check traffic signal
	if( get_light( IS_2, dir, newLane ) == RED ) {
		// Put vehicle in queue
		set_velocity( V, 0.0 );
		if( newLane == get_numLanes( IS_2 )[dir] && IS_2_red_right_turn( dir ) ) {
			Event entering = peek_from_list( get_lane_queue( IS_2, dir, newLane ) );
		...
		if(   get_list_counter( get_lane_queue( IS_2, dir, newLane ) ) == 1
		   && get_lane_flag( IS_2, dir, newLane ) == 0 )
		{
...
static void IS_2_entering( void* P ) {
	...
	if( get_lane_flag( IS_2, dir, laneID ) == 0 ) {
		set_event_type( E, IS_2_CROSSING );
		set_callback( E, IS_2_crossing );
		...
		set_lane_flag( IS_2, dir, laneID, 1 );
		change_lane_counter( IS_2, dir, laneID, 1 );
		schedule_event( poll_from_list( get_lane_queue( IS_2, dir, laneID ) ) );
		add_wait_time( V, get_sim_time()-get_wait_time_buf( V ) );
		set_wait_time_buf( V, 0.0 );
		// Decrement section vehicle counter
		if( dir == SOUTH ) change_north_vehicles( S_2, -1 );
		if( dir == NORTH ) change_south_vehicles( S_3, -1 );
	}
}
...
static void IS_2_departure( void* P ) {
	...
	change_lane_counter( IS_2, dir, get_laneID(V), -1 );
	...
	if( get_route( V ) != LEFT ) {
		...
		if( IS_2_left_turn( opposite ) == 1 ) {
			Event entering = peek_from_list( get_lane_queue( IS_2, opposite, 1 ) );
			set_timestamp( entering, get_sim_time() );
			schedule_event( entering );
		}
	}
	...
	if( IS_2_red_right_turn( redTurn ) ) {
		Event rightTurn = peek_from_list( get_lane_queue( IS_2, redTurn, get_numLanes( IS_2 )[redTurn] ) );
